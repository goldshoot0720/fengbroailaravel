...
- **Storage 檔案管理雙向對帳**：除了原本的「uploads 有檔案、資料庫沒引用」，設定頁再加一段資料庫殘餘紀錄清理，涵蓋鋒兄圖片／影片／音樂／播客／文件，分三類：**沒有檔案來源**（`file`／`cover` 皆空白）與**檔案已消失**（`file` 指向的 uploads 檔案不見）可整筆刪除，**僅封面失效**只清掉 `cover` 欄位保留紀錄。外部網址欄位不列入；刪除前會標出該筆是否還留有歌詞／備註／連結／分類（例如音樂常是「檔案沒了、歌詞還在」），清理時前後端都會重新驗證
- **底部分頁分類記憶上次子選單**：行動版「生活／媒體／工具」底欄分頁不再永遠回到分類第一項，改對這次點進去的頁面（對齊 Appwrite 上途列表選單分類級記憶）
- **鋒兄常用可搜尋式網站選取器**：新增／編輯常用帳號的網站欄位改用 `datalist` 原生搜尋，取代舊的選單+自行輸入雙欄位切換
- **工具個人清單雲端同步**：手動價格改存伺服器 `manualprice` 表（跨瀏覽器同步，localStorage 降為離線快取與首次遷移來源）；Tube 頻道改存 `tubechannel` 表（舊 JSON 首次自動遷移）；金融自訂標的同步寫入 `financeinstrument` 表
- **通知設定密碼保護**：設定頁可建立可選通知密碼；之後儲存／變更 RESEND／BigGo API 金鑰都需驗證（Cron 自動寄信／推播不受阻擋）
- **Resend 設定 CSV 匯入／匯出**：驗證通知密碼後可匯出金鑰＋收件 Email CSV；匯入先解析預覽，依收件 Email 合併（同 Email 覆蓋、新 Email 補位）再寫回
- **鋒兄購物清單**：獨立 `shoppinglist` 表，記錄「商品 × 一次預定購買」；CRUD／複製／刪除確認／篩選（3 天內、今天、已過期、未設定日期）／CSV 匯入匯出
- **統一五模組到期提醒窗口**：訂閱、試用／首購、購物清單、額度非 AI 剩 0~3 天；食品 0~7 天；額度 AI 一週／一月只提醒前一天與當天。完整儀表彙整五模組到期統計並每日本機通知，Web Push 改為彙總訊息
- **儀表板合併進首頁**：鋒兄首頁同一頁切換「精簡／完整儀表」，記住上次選擇；獨立「儀表」選單移除（舊網址自動落回首頁完整儀表）
- **網站統計**：進站人次、連續進站天數（台北日曆日）、選單使用 Top 5、銀行總存款歷史極值與網站營運天數（自 2025-09-28 起算）；`sitevisit` / `menuusage` 表，設定頁可初始化
...
- 關於頁程式碼統計即時計算 + 對齊更新說明
- **鋒兄試用／首購**：獨立 `trialpurchase` 表，依服務分組帳號、試用／首購狀態、CSV 匯入匯出
- **鋒兄重灌**：獨立 `reinstall` 表，Windows／Mac 軟體清單、付費序號遮罩與查看密碼
...
**可移植功能模組已標記完成**（2026），含試用／首購與重灌。

近期補完：鋒兄試用／首購、鋒兄重灌、金融標的「解析名稱」、手機歷史 CSV 匯入、Whisper 語言選擇、語音導向各工具子頁。
