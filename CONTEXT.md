...
- After a single `api.php` create/update succeeds (not `fengbroQuiet` ones), the item is remembered in sessionStorage and, once the soft reload or the full reload finishes, scrolled to about a quarter below the top of the viewport, highlighted and focused (collapsed `.mgmt-group` / `<details>` ancestors are opened first). Batches that write several items do not scroll. `ux-boost.js` skips its scroll restore while such a focus is pending.
