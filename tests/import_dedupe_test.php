...
// 10. 一般匯入會復原訂閱／筆記的軟刪除資料，但不影響其他表
check(
    '訂閱匯入復原垃圾桶資料',
    true,
    fengbroImportRestoresSoftDeletedRows('subscription', ['id', 'name', 'deleted_at'])
);
check(
    '沒有垃圾桶欄位不強制復原',
    false,
    fengbroImportRestoresSoftDeletedRows('subscription', ['id', 'name'])
);
check(
    '其他資料表不套用垃圾桶復原',
    false,
    fengbroImportRestoresSoftDeletedRows('food', ['id', 'name', 'deleted_at'])
);
