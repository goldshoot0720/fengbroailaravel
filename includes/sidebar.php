...
function rememberLastMenu(){const page=document.body.dataset.page||'';if(!page||page==='home')return;let tool=document.body.dataset.tool||'';if(!/^[a-z0-9-]+$/i.test(tool))tool='';const url=(tool?'page='+page+'&tool='+encodeURIComponent(tool):'page='+page);localStorage.setItem('fengbro_last_menu',url);rememberLastGroupMenu(url);}
...
// 每個底部分頁分類自己記住上次點進去的子選單（生活/媒體/工具），不再永遠回到該分類第一項。
const FENGBRO_GROUP_SETS={life:['subscription','trialpurchase','reinstall','quota','shoppinglist','food','bank','routine'],media:['images','videos','music','podcast'],tools:['tools']};
function fengbroGroupForPage(page){for(const g in FENGBRO_GROUP_SETS){if(FENGBRO_GROUP_SETS[g].includes(page))return g;}return null;}
function rememberLastGroupMenu(url){const m=String(url||'').match(/page=([a-z0-9-]+)/i);if(!m)return;const group=fengbroGroupForPage(m[1].toLowerCase());if(group)localStorage.setItem('fengbro_last_group_'+group,url);}
function handleMenuNav(link){closeMobileMenu();const m=link.dataset.menuUrl;if(!m)return;if(m==='page=home'){localStorage.removeItem('fengbro_last_menu');}else{localStorage.setItem('fengbro_last_menu',m);rememberLastGroupMenu(m);}recordMenuUsage(fengbroModuleIdFromUrl(m));}
function handleGroupTabNav(event,link){const group=link.dataset.group;if(group){const remembered=localStorage.getItem('fengbro_last_group_'+group);if(remembered&&remembered!==link.dataset.menuUrl){event.preventDefault();closeMobileMenu();localStorage.setItem('fengbro_last_menu',remembered);recordMenuUsage(fengbroModuleIdFromUrl(remembered));window.location.href='index.php?'+remembered;return false;}}handleMenuNav(link);return true;}
restoreLastMenu();
rememberLastMenu();
